...
export default function Builder() {
    ...
    const {skillPoints, level} = decodeHash(localStorage.getItem('v'), localStorage.getItem('hash'));
    ...
    return <>
        ...
        <section id={"left"}>
            <div class={"equipments"}>
                <Entries of={builderData.equipments}>{(key, value) => <div className={`${key}`}>
                    <For each={value()}>{(s, i) => <Equipment type={s.type} index={s.index ?? -1}></Equipment>}</For>
                </div>}</Entries>
            </div>
            <div class={"skills"}>
