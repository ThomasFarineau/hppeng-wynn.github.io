...
export function createModal() {
    ...
    return {
        ...
        Modal(props) {
            ...
            const [maxLevel, setMaxLevel] = createSignal(data.builderData.maxLevel);
            ...
            const handleBlur = (e) => {
                if (ulElement && !ulElement.contains(e.relatedTarget)) setIsFocused(false);
            };
                                ...
                                <div class="filters">
                                    <label>
                                        ...
                                        <input
                                            type="range"
                                            min={0}
                                            max={data.builderData.maxLevel - 1}
                                            value={minLevel()}
                                            style={
                                                "background-size: " +
                                                (Number(minLevel()) /
                                                    (data.builderData.maxLevel - 1)) *
                                                100 +
                                                "% 100%"
                                            }
                                            onInput={(e) => {
                                    ...
                                    <label>
                                        ...
                                        <input
                                            type="range"
                                            min={1}
                                            max={data.builderData.maxLevel}
                                            value={maxLevel()}
                                            style={
                                                "background-size: " +
                                                (Number(maxLevel()) / data.builderData.maxLevel) *
                                                100 +
                                                "% 100%"
                                            }
                                            onInput={(e) => {
                                    ...
                                    <label>
                                        ...
                                        <input
                                            type="range"
                                            min={-1}
                                            max={data.builderData.maxPowderSlots}
                                            style={
                                                "background-size: " +
                                                ((Number(powderSlots()) + 1) /
                                                    (data.builderData.maxPowderSlots + 1)) *
                                                100 +
                                                "% 100%"
                                            }
                                            value={powderSlots()}
                                    ...
                                    <span class={"tags"}>
                                        <Entries of={data.TierEnum}>
                                            {(key, v) => (
                                                <div class={`tag ${v()} ${selectedTiers().includes(v()) ? "selected" : ""}`}
                                        ...
                                        <For each={items()}>
                                            {(item) => (
                                                <li class={item.tier} onClick={() => onItemClick(item)}>
                                                    <div class={`icon ${item.type ?? item.accessoryType}`}></div>
                                                    {item.name}
                                                </li>
                                            )}
                                        </For>
