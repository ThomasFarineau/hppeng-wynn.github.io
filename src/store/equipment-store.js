...
function updateBonusAndMandatory() {
  ...
  _.map(equipmentStore.items, (item) =>
    _.map(
      equipmentStore.bonus,
      (value, key) =>
        (newBonus[key] += _.get(
          item,
          `identifications.${getBonusName(key)}`,
          0
        ))
    )
  );
  ...
  _.map(
    equipmentStore.mandatory,
    (value, key) =>
      (newMandatory[key] += _.max(
        _.map(equipmentStore.items, (item) =>
          _.get(item, `requirements.${key}`, 0)
        )
      ))
  );
...
export {equipmentStore, bonusAndMandatorySignal};
